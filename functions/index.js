...
exports.receiveCallDetails = functions.https.onRequest(
  async (request, response) => {
    ...
    try {
      ...
      const snapshot = await usersRef
        .where("viciUsername", "==", callData.agent_user_id)
        .get();
...
exports.deleteExpiredDocs = functions.pubsub
...
exports.updateAndDeleteDocTempURL = functions.https.onCall(
  async (data, context) => {
...
exports.generateTempURL = functions.https.onCall(async (data, context) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!reportDocSnapshot.exists) {
      throw new functions.https.HttpsError(
        "not-found",
        "Report document does not exist",
      );
    }
...
exports.searchByName = functions.https.onCall((data, context) => {
  ...
  if (!data.name) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      'The function must be called with one argument "name".',
    );
  }
...
exports.addPhoneSuffixToReports = functions.https.onRequest(
  async (req, res) => {
    const reportsRef = admin.firestore().collection("reports");
...
exports.receiveAssignedAgent = functions.https.onRequest(async (req, res) => {
  ...
  const allowedOrigin = "2600:1900:20";
  ...
  try {
    const usersRef = admin.firestore().collection("users");
    const snapshot = await usersRef
      .where("id", "==", req.body.reportAgentId)
      .get();

    if (snapshot.empty) {
      return res
        .status(404)
        .send("No user found with the given reportAgentId.");
    }
    ...
    return res.send({
      result: "Lead data received and saved with timestamp to Firestore",
    });
  } catch (error) {
